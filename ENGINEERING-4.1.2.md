...
1. `FoldReels.render()` élargissait le viewer au `getGlobalVisibleRect(android.R.id.content)`. Ce rectangle appartient au contenu visible et peut conserver des insets/layouts natifs. Masquer les barres système ne garantit pas sa coïncidence avec le rectangle de la fenêtre. Le code n’avait pas de repère explicite de fenêtre pour le dessin full-bleed. `getGlobalVisibleRect` fournit en outre des coordonnées relatives à la racine, tandis que `transformMatrixToGlobal` utilise l’écran ; l’origine de fenêtre est maintenant convertie explicitement avant toute intersection. Sa frontière de clipping s’arrêtait aussi avant ce contenu : augmenter un target ne débloquait pas cette dernière frontière.
